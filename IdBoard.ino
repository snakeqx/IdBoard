...
int i2cSpeed = 400000;
int totalMenu = 9;
String menu[] = {"Scan", "Read", "Dump", "Clear", "Write Dump Back", "Write New", "Write Old", "Transfer Dump", "data to pc"};
int E2PROM = 256;
int menuSelect = 0;
...
void transferDump()
{
    ...
    delay(100);
    int data_len = 0;
    //arduino serial buffer size = 64 bytes
    while(data_len<E2PROM)
    {
        while(Serial.available()<64){}
        for(int i=0;i<64;i++)
        {
            data[i+data_len]=Serial.read();
        }
        data_len+=64;
        if(data_len<E2PROM) Serial.print("data partial received\n");
        else Serial.print("data received completely\n");
    }
    ...
    if (!file)
    {
        M5.Lcd.println("Failed to write file to dump.dll");
        return;
    }

    for (int i = 0; i < E2PROM; i++)
    {
        if (file.print((char)data[i]))
        {
        }
        else
        {
            M5.Lcd.println("Writing failed!");
            break;
        }
    }
...
void dataToPc()
{
    Serial.flush();
    clearScreen();
    M5.Lcd.println("read data from EEPROM");
    ...
    if (str == "please send")
    {
        for(int i=0;i<E2PROM;i++) Serial.print((char) data[i]);
    }
